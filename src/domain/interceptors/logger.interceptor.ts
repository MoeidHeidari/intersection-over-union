...
export class LoggerInterceptor implements NestInterceptor {
     ...
     * @returns nothing
     */
    private logKafkaMessage(context: ExecutionContext, startTime: number,
        reqTime: number, controllerName: string, handlerName: string, args: any) {

        for (const arg of args) {
            if (!arg.args) {
                this.logger.log(
                    `[KAFKA] ${arg.partition} ${arg.topic} [${controllerName}:${handlerName}] ${reqTime}ms`
                );
            }
        }
    }
     ...
     * @returns nothing
     */
    private logMqttMessage(context: MqttContext, startTime: number,
        reqTime: number, controllerName: string, handlerName: string) {

        const packet = context.getPacket();
        this.logger.log(
            `[MQTT] ${packet.cmd} ${context.getTopic()} [${controllerName}:${handlerName}] ${reqTime}ms`
        );
    }
